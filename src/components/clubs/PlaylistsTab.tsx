...
import {
  ...
  PlaySquare, Clock, Pencil, X, Check,
} from 'lucide-react';
...
function PlaylistCard({
  ...
  const handleAddVideo = async () => {
    ...
    setAddingVideo(true);
    const thumb = autoThumbnail(vUrl.trim()) ?? null;
  ...
  const handleSaveEdit = async () => {
    ...
    const { error } = await supabase.from('club_playlists').update({
      ...
      cover_emoji: editEmoji.trim() || '📂',
    }).eq('id', playlist.id);
    if (error) { toast.error('Could not save changes.'); }
    else {
      onUpdate({ ...playlist, title: editTitle.trim(), description: editDesc.trim() || null, accent_color: editColor, cover_emoji: editEmoji.trim() || '📂' });
      setEditing(false);
...
export default function PlaylistsTab({ clubId, userId, canManage, isMember }: PlaylistsTabProps) {
  ...
  const handleCreate = async () => {
    ...
    if (error) { toast.error('Could not create playlist.'); }
      ...
      setFTitle(''); setFDesc(''); setFColor('#8B7BFF'); setFEmoji('📂');
      setShowForm(false);
      ...
      {showForm && (
        <div className="sc-card p-4 flex flex-col gap-3">
          ...
          <div className="rounded-xl overflow-hidden border-l-4 p-3 flex items-center gap-3"
            ...
            <span className="text-xl">{fEmoji || '📂'}</span>
            <div>
