    ...
    supabase
      ...
      .then(({ data }) => {
        if (data) {
          setRows(data.map(r => ({
            ...r,
            profile: (Array.isArray(r.profile) ? r.profile[0] : r.profile) as PointProfile | null | undefined,
          })));
        }
        setLoading(false);
      });
    ...
    <div className="flex flex-col gap-5">
      ...
      {showInfo && (
        <div className="sc-card p-4 grid grid-cols-2 gap-2 text-xs text-[var(--color-text-secondary)]">
          <div className="flex items-center gap-2">
            ...
            <span><strong className="text-[var(--color-text)]">+2 pts</strong> — Chat message</span>
          </div>
          <div className="flex items-center gap-2">
            <Zap className="w-3.5 h-3.5 text-[var(--color-amber)]" />
            <span><strong className="text-[var(--color-text)]">+15 pts</strong> — Launch project</span>
          </div>
          <div className="flex items-center gap-2">
            <Zap className="w-3.5 h-3.5 text-[var(--color-amber)]" />
            <span><strong className="text-[var(--color-text)]">+12 pts</strong> — Project accepted</span>
          </div>
          <div className="flex items-center gap-2">
            <Zap className="w-3.5 h-3.5 text-[var(--color-amber)]" />
            <span><strong className="text-[var(--color-text)]">+4 / +6 / +20</strong> — Quest join / step / complete</span>
          </div>
          <div className="flex items-center gap-2">
            <Zap className="w-3.5 h-3.5 text-[var(--color-amber)]" />
            <span><strong className="text-[var(--color-text)]">+10 pts</strong> — Event RSVP</span>
          </div>
          <div className="flex items-center gap-2">
            <Zap className="w-3.5 h-3.5 text-[var(--color-amber)]" />
            <span><strong className="text-[var(--color-text)]">+5 / +1</strong> — Feed post / reaction</span>
          </div>
        ...
        <div className="sc-card p-4 border-l-4 border-l-[var(--color-amber)]">
          ...
          <div className="flex items-center gap-3">
            ...
            <Avatar className="w-8 h-8">
              ...
              <AvatarFallback className="text-xs">{initials(myRow.profile)}</AvatarFallback>
            </Avatar>
      ...
      {rows.length === 0 ? (
        <div className="sc-card p-10 text-center text-[var(--color-text-secondary)]">
          ...
          <p className="text-xs mt-1">Chat, projects, quests, and events all count toward contribution points.</p>
        </div>
      ) : (
            ...
            return (
              <div key={row.user_id}
                ...
                <Avatar className="w-8 h-8 flex-shrink-0">
                  ...
                  <AvatarFallback className="text-xs">{initials(row.profile)}</AvatarFallback>
                </Avatar>
